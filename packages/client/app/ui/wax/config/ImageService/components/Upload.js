...
const findPlaceholder = (state, id, placeholderPlugin) => {
  ...
  const found = pluginState.decorations.find(null, null, spec => spec.id === id);
...
const findSafeInsertionPos = (state, targetPos) => {
  ...
  try {
    ...
    if (resolved.parent.type.isTextblock || resolved.parent.type.name === 'paragraph') {
      return pos;
    }
  } catch (e) {
    // Position invalid
...
export default (view, fileUpload, placeholderPlugin, context, app) => file => {
  ...
  console.log(`Starting upload for placeholder ${placeholderId} at position ${initialPos}`);
  ...
  fileUpload(file).then(
    fileData => {
      ...
      if (insertPos === null) {
        const trackedPlaceholder = findPlaceholderInTracking(state, placeholderId, placeholderPlugin);
        if (trackedPlaceholder) {
          console.log(`Decoration missing but placeholder tracked, finding safe position near ${trackedPlaceholder.originalPos}`);
          insertPos = findSafeInsertionPos(state, trackedPlaceholder.originalPos);
        }
      }
      ...
      console.log(`Inserting image at position ${insertPos} for placeholder ${placeholderId}`);
      ...
      try {
        ...
        if (parent.type.name === 'paragraph' && parent.content.size === 0) {
          ...
          const paragraphWithImage = schema.nodes.paragraph.create({}, imageNode);
        ...
        console.log(`Successfully inserted image for placeholder ${placeholderId}`);
