...
const getMessages = async (channelId, options = {}) => {
  ...
  try {
    const messages = await ChatMessage.query(options.trx).where(
      'chatChannelId',
      channelId,
    )
    ...
    if (messageIds.length > 0) {
      try {
        ...
        logger.info(
          `${CONTROLLER_MESSAGE} Found ${allAttachments.length} attachments`,
        )
      } catch (attachmentError) {
        logger.error(
          `${CONTROLLER_MESSAGE} Error fetching attachments: ${attachmentError.message}`,
        )
        allAttachments = []
      }
    }
...
const getAttachments = async ({ id }) => {
  try {
    ...
    if (error.message.includes('too many clients already')) {
      logger.info(
        `${BASE_MESSAGE} getAttachments: Retrying after connection pool error`,
      )
      ...
      try {
        ...
        logger.error(
          `${BASE_MESSAGE} getAttachments retry failed: ${retryError.message}`,
        )
        return []
      }
    }
